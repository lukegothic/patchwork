...
class Game extends Component {
    constructor(props) {
        ...
        const startingPlayer = shuffle(playerNames.slice())[0];
    ...
    testREF = () => {
        this.setState(state => {
            ...
            console.log(players);
            let ps = this.getPlayers(players);
            ps.active.money = 200;
            return {
                players: players
            }
    ...
    */
    ...
    performEndTurnActions = () => {
        this.setState(state => {
            ...
            // 0. Si el jugador se ha pasado del final, ponerlo al final
            playersByStatus.active.position = clamp(playersByStatus.active.position, state.cfg.timeboard.size - 1);
            ...
            if (checkpoint) {
                switch(checkpoint.type) {
                    ...
                    break;
                    case "patch":
                        let cp = checkpoints.find(cp => cp.id === checkpoint.id);
                        cp.pickedup = true;
            ...
            // Si el jugador ha cogido un patch de bonus, sigue siendo su turno
    ...
    handlePass = () => {
        this.setState(state => {
            ...
            const advance = playersByStatus.other.position - playersByStatus.active.position + 1;
    ...
    handlePlacePatch = (at) => {
        this.setState(state => {
            ...
            if (this.isLegalPlacement(playersByStatus.active, state.patchIntent, at)) {
                ...
                playersByStatus.active.position_before = playersByStatus.active.position;
                playersByStatus.active.position += state.patchIntent.cost.time;
    ...
    render = () => {
        return (<div className="game">
            {(this.state.players.reduce((acc, curr) => acc + curr.position, 0) === (this.state.cfg.timeboard.size - 1) * this.state.players.length) && <EndGame players={this.state.players} />}
            <div className="main">
                { this.state.players.map((player, i) => <Player key={player.name} size={this.state.cfg.playerboard.size} player={player} patch={this.state.patchIntent} onPlacePatch={this.handlePlacePatch} />) }
                {this.state.patchIntent !== null
                    ...
                    : <Market playerMoney={this.state.players.find(p => p.playing).money} patches={this.state.patchList} onBuyPatch={this.handleBuyPatchIntent} />
                }
